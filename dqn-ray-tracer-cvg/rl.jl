...
function sample_batch(memory::Memory, batch_size::Int, actions_size::Int)
    ...
    sampled_states = zeros(6, batch_size)
    sampled_rewards = zeros(1, batch_size)
    sampled_mse_rewards = zeros(1, batch_size)
    ...
    for i=1:batch_size
        ...
        sampled_mse_rewards[1,i] = memory.quads[sampled_indices[i]].mse_reward
        sampled_actions[1,i] = memory.quads[sampled_indices[i]].action + (i-1)*actions_size
    end
    return vcat(sampled_states, sampled_rewards, sampled_mse_rewards), sampled_actions
end
