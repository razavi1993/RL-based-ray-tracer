...
img, img_history, t_steps = render(world, cam, model, target_model, q_net_ps, target_net_ps, model_ps, 8192, loss, 12, 30, nx, ny, ns)
imshow(img)
save("path\\dqn-ray-tracer-cvg-rbf\\img.png", img)
loss_values = t_steps.loss_values
loss_values = reshape(mean(reshape(loss_values, 20,50), dims=1), 50)
plot(collect(1:50), loss_values)
savefig("path\\dqn-ray-tracer-cvg-rbf\\loss_values.png")

npzwrite("path\\dqn-ray-tracer-cvg-rbf\\img_history.npz", img_history)
npzwrite("path\\dqn-ray-tracer-cvg-rbf\\loss_values.npz", loss_values)
