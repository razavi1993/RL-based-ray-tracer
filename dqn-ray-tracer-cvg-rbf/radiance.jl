...
# radiance function
function radiance(r::ray, world::hitable, depth::Int, memory::Memory, nu::Int, nv::Int, batch_size, loss, model, target_model, q_network_params, target_network_params, model_params, opt::OptimizerParams, t_steps::TrainingSteps)
    put = vec(1,1,1)
    local state, action
    for i=1:depth
        ...
        if !ismissing(hit_surface)
            if typeof(hit_surface.mater) == lambertian
                ...
                qvalues = reshape(model(next_state), (nu,nv))
                ...
                if i > 1 && t_steps.current_step < t_steps.max_steps
                    target_qvalues = reshape(target_model(next_state), (nu,nv))
                    mse_target_qvalues = reshape(model(next_state), (nu,nv))
                    ...
                    if memory.min_length <= length(memory.quads)
                        ...
                        train_network(loss, q_network_params, target_network_params, model_params, inputs, indices, memory, opt, t_steps, batch_size)
                    end
                end
                state = next_state
                cosθ = dot(ω, n)
