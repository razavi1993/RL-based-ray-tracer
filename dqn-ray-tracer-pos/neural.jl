...
# q-network
q_net = fmap(f64, Chain(Dense(54, 1000, relu), Dense(1000, 108, softplus)))
...
function loss(input, indices)
    x, target_values = input[1:54,:], input[55:55,:]
...
# positional encoding
function encode_position(x)
    k = 2 .^collect(0:7)
    y = vcat(sin.(pi*k*x'), cos.(pi*k*x'))
