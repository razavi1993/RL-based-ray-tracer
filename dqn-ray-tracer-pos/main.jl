...
imshow(img)
save("path\\dqn-ray-tracer-pos\\img.png", img)
loss_values = t_steps.loss_values

loss_values = reshape(mean(reshape(loss_values, 20,50), dims=1), 50)
plot(collect(1:50), loss_values)
savefig("path\\dqn-ray-tracer-pos\\loss_values.png")

npzwrite("path\\dqn-ray-tracer-pos\\img_history.npz", img_history)
npzwrite("path\\dqn-ray-tracer-pos\\loss_values.npz", loss_values)
